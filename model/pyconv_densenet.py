...
class _DenseLayer(nn.Module):
    def __init__(
        self, num_input_features: int, growth_rate: int, bn_size: int, drop_rate: float, memory_efficient: bool = False, pyconv_kernels=[3], pyconv_groups=[1]
    ) -> None:
        super().__init__()
        ...
        # )
        self.add_module(
            "conv2", get_pyconv(bn_size * growth_rate, growth_rate, pyconv_kernels=pyconv_kernels, stride=1,
...
class DenseNet(nn.Module):
    ...
    def __init__(
        ...
        for i, num_layers in enumerate(block_config):
            ...
            if i == 0:
                self.features.add_module("denseblock%d" % (i + 1), block1)
            elif i == 1:
                self.features.add_module("denseblock%d" % (i + 1), block2)
            elif i == 2:
                self.features.add_module("denseblock%d" % (i + 1), block3)
            elif i == 3:
                self.features.add_module("denseblock%d" % (i + 1), block4)
...
def _densenet(
    ...
    **kwargs: Any,
    ...
    if pretrained:
        ...
        predict_model = torch.load('model/densenet121-a639ec97.pth')
...
def pyconv_densenet121(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> DenseNet:
...
if __name__ == '__main__':
    model = pyconv_densenet121(pretrained = True)
    model.cuda()
    # print(model)
